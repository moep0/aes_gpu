...
printf("CUDA error:133 %s\n", cudaGetErrorString(error2));
printf("done\n");
  //end = clock();
  cudaEventRecord(stop1, NULL);
  error2=cudaGetLastError();
printf("CUDA error134.3: %s\n", cudaGetErrorString(error2));
...
如我们将利用上图2-dim结构实现两个矩阵的加法，每个线程负责处理每个位置的两个元素相加，
...
printf("CUDA error233: %s\n", cudaGetErrorString(error2));
  ...
  printf("%d %d\n",buf[0],buf[2]);
  //FILE *file;
  //file = fopen("cipher2.txt", "w");
  //fwrite(buf_d, 1, numbytes, file);
  //printf("yes\n");
  //fclose(file);  
...
int main(){
  ...
  cudaEventElapsedTime(&msecTotal1, start1, stop1);
  total=msecTotal1/1000;
  //cout<<"time used: ! "<<total<<endl;
  long r=1<<27; //单位换算常数
  //cout<<"GPU decryption throughput:"<<block_number*BLOCK_LEN/total/r<<" Gbps"<<endl;
  printf("time used:%f\n",  (double)total);
